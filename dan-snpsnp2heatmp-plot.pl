...
$lgg="LG".$m;
$infile="C:\\RosbreedData\\dan-snpsnp-count-$lgg-oldFii.csv";
$rscript="Heatmap-draw-$lgg.r";
open(OUT,">>$rscript");
print OUT "pdf(file = 'dan-snpsnp-count-$lgg.pdf')\n";
print OUT "plot.new()\n";
print OUT "plot.window(c(0,12000),c (-10000,3200))\n";
...
while(defined($line=<IN>)){
	...
	for($i=0;$i<scalar@temp;$i++){
		...
		$y2=3000-$yspace-300;
		#$color=$lightcolor;
		if($l>2 and $cell<0.3 and $cell=~/\d/){
			#$color=$color;
			$color=$lightcolor[0];
			print OUT "boxcolour<-'$color'\n rect($x1,$y1,$x2,$y2,col=boxcolour, border=NA)\n";
		}elsif($l>2 and $cell<0.6 and $cell=~/\d/){
			#$color=$color."3";
			$color=$lightcolor[1];
			print OUT "boxcolour<-'$color'\n rect($x1,$y1,$x2,$y2,col=boxcolour, border=NA)\n";
		}elsif($l>2 and $cell<1.1 and $cell=~/\d/){
			#$color=$color."4";
			$color=$lightcolor[2];
			print OUT "boxcolour<-'$color'\n rect($x1,$y1,$x2,$y2,col=boxcolour, border=NA)\n";
		}	
		...
		#}else{
			print OUT "text(120+$xspace,3000-$yspace,'$cell',cex=0.5)\n";
		#}	
